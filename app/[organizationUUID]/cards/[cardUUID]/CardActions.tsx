...
export default function CardActions({ orgUUID, cardUUID, initialText }: { orgUUID: string; cardUUID: string; initialText: string }) {
  // WHAT: Edit updates card text; Archive performs a soft-delete by setting isArchived=true; Delete removes the card.
  // WHY: Archive preserves history and removes the card from active lists, aligning with soft-delete patterns and cross-tab sync.
  ...
  async function onArchive() {
    ...
    } catch (e) { alert(e instanceof Error ? e.message : 'Archive failed') }
  }
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <button onClick={onArchive} className="px-3 py-1 text-sm rounded bg-yellow-50 hover:bg-yellow-100 text-yellow-700 border border-yellow-200">Archive</button>
