...
export default function Board({ boardSlug, initialView = 'kanban', axisHidden = false, titleOverrides = {} as Partial<Record<'do' | 'decide' | 'delegate' | 'decline', string>> }: { boardSlug: string, initialView?: 'kanban' | 'matrix', axisHidden?: boolean, titleOverrides?: Partial<Record<'do' | 'decide' | 'delegate' | 'decline', string>> }) {
  ...
  const load = useCallback(async () => {
    const qs = `&board=${encodeURIComponent(boardSlug)}`
    const [r, b, t, d] = await Promise.all([
      fetchJSON<Card[]>(`/api/cards?status=delegate${qs}`),
      fetchJSON<Card[]>(`/api/cards?status=decide${qs}`),
      fetchJSON<Card[]>(`/api/cards?status=do${qs}`),
      fetchJSON<Card[]>(`/api/cards?status=decline${qs}`),
    ])
    setRoadmap(r)
    setBacklog(b)
    setTodo(t)
  ...
  useEffect(() => {
    const handler = () => { load() }
    try { window.addEventListener('card:created', handler) } catch {}
    return () => { try { window.removeEventListener('card:created', handler) } catch {} }
  }, [load])
...
export function CardItem({ card, index, status, onUpdate, onDelete, onArchive, bubbleContext, onHoverIndex, onDragFlag, hideBadges = false, extraChips, styledChips }: {
  card: Card
  index: number
  status: Card['status']
  ...
  hideBadges?: boolean
  extraChips?: string[]
  styledChips?: { text: string; bg: string; fg: string }[]
}) {
    ...
    >
      ...
      <div className="mt-2 flex items-center justify-between text-xs text-gray-700">
        <div className="flex items-center gap-2 flex-wrap">
          {/* Styled chips have explicit colors (e.g., board area labels), shown first */}
          {Array.isArray(styledChips) && styledChips.map((sc, i) => (
            <span key={`sc-${i}`} className="px-2 py-0.5 rounded-full text-[10px] font-mono" style={{ backgroundColor: sc.bg, color: sc.fg }}>{sc.text}</span>
          ))}
          {/* Legacy extraChips fall back to theme mappings */}
          {Array.isArray(extraChips) && extraChips.map((chip, i) => {
            ...
            return (
              <span key={`xc-${i}`} className="px-2 py-0.5 rounded-full text-[10px] font-mono" style={{ backgroundColor: bg, color: fg }}>{chip}</span>
            )
          })}
          {!hideBadges && (
            <>
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={async () => { await onDelete(card.id) }}
