...
import { CardItem } from '@/components/Board'
...
{list.map((c, idx) => {
                ...
                return (
                  <CardItem
                    ...
                    onUpdate={onUpdateUi}
                    onDelete={onDeleteUi}
                    onArchive={onArchiveUi}
                    onHoverIndex={(t) => setTarget({ area, index: t.index })}
                    bubbleContext={{ kind: 'decide', ...areaStats }}
                    onDragFlag={(d) => setDragging(d ? { id: d.id, from: area } : null)}
